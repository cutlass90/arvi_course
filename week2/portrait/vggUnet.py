...
def VGGUnet(config):
    img_input = Input(shape=(config.data.img_height, config.data.img_width, 3))
    vgg_level = 3
    ...
    d = Conv2D(1, (1, 1))(d)
    d = Lambda(Interp, arguments={'shape': (config.data.image_shape[0],
                                            config.data.image_shape[1])})(d)
    d = Reshape([config.data.image_shape[0], config.data.image_shape[1]])(d)
